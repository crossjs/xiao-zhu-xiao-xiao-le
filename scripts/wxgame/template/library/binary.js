      ...
      if (path.isRemotePath(xhrURL)) {
        if (needCache(xhrURL)) {
          ...
          if (fs.existsSync(targetFilename)) {
            // 缓存命中
            const data = WXFS.readFileSync(path.getWxUserPath(targetFilename));
            resolve(data);
          } else {
            loadBinary(xhrURL)
              .then(content => {
                ...
                const needRead = needReadFile();
                if (needRead) {
                  content = WXFS.readFileSync(
                    path.getWxUserPath(targetFilename)
                  );
                }
                resolve(content);
              })
...
function needReadFile() {
  ...
  const platform = wxSystemInfo.system.split(" ").shift();
