...
export class BricksPlugin implements plugins.Command {
  ...
  public async onFile(file: plugins.File) {
    ...
    if (filename === "manifest.json") {
      ...
      let result = content.replace(
        /RES\.loadConfig\("resource\/default\.res\.json", "resource\/"\)/gm,
        'RES.loadConfig("GameRes://resource/default.res.json", "GameRes://resource/")',
      );
      result = result.replace(
        /eui\.Theme\("resource\/default\.thm\.json", _this\.stage\)/gm,
  ...
  public async onFinish(pluginContext: plugins.CommandContext) {
    ...
    const mainJSPath = path.join(pluginContext.outputDir, "main.js");
