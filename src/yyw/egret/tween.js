var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var yyw;
(function (yyw) {
    var PromisedTween = /** @class */ (function () {
        function PromisedTween(target, loop) {
            if (loop === void 0) { loop = false; }
            this.tween = egret.Tween.get(target, {
                loop: loop,
            });
        }
        PromisedTween.prototype.to = function (props, duration, ease) {
            var _this = this;
            if (duration === void 0) { duration = 100; }
            if (ease === void 0) { ease = egret.Ease.quadOut; }
            // @todo 使用计算
            // Object.entries(props).forEach(([key, value]) => {
            //   if (typeof value === "string" && /^[+-]/.test(value)) {
            //   }
            // });
            return new Promise(function (resolve) {
                _this.tween.setPaused(false).to(props, duration * yyw.CONFIG.speedRatio, ease).call(resolve);
            });
        };
        return PromisedTween;
    }());
    function removeAllTweens() {
        egret.Tween.removeAllTweens();
    }
    yyw.removeAllTweens = removeAllTweens;
    function pauseTweens(target) {
        egret.Tween.pauseTweens(target);
    }
    yyw.pauseTweens = pauseTweens;
    function resumeTweens(target) {
        egret.Tween.resumeTweens(target);
    }
    yyw.resumeTweens = resumeTweens;
    function removeTweens(target) {
        egret.Tween.removeTweens(target);
    }
    yyw.removeTweens = removeTweens;
    function getTween(target, loop) {
        if (loop === void 0) { loop = false; }
        return new PromisedTween(target, loop);
    }
    yyw.getTween = getTween;
    function fadeIn(target, duration, ease) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        target.alpha = 0;
                        target.visible = true;
                        return [4 /*yield*/, getTween(target).to({
                                alpha: 1,
                            }, duration, ease)];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    }
    yyw.fadeIn = fadeIn;
    function fadeOut(target, duration, ease) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, getTween(target)
                            .to({
                            alpha: 0,
                        }, duration, ease)];
                    case 1:
                        _a.sent();
                        target.visible = false;
                        target.alpha = 1;
                        return [2 /*return*/];
                }
            });
        });
    }
    yyw.fadeOut = fadeOut;
    function zoomIn(target, duration, ease) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        target.alpha = 0;
                        target.scale = 0;
                        target.visible = true;
                        return [4 /*yield*/, getTween(target).to({
                                alpha: 1,
                                scale: 1,
                            }, duration, ease)];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    }
    yyw.zoomIn = zoomIn;
    function zoomOut(target, duration, ease) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, getTween(target)
                            .to({
                            alpha: 0,
                            scale: 0,
                        }, duration, ease)];
                    case 1:
                        _a.sent();
                        target.visible = false;
                        target.alpha = 1;
                        target.scale = 1;
                        return [2 /*return*/];
                }
            });
        });
    }
    yyw.zoomOut = zoomOut;
    function twirlIn(target, duration, ease) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        target.alpha = 0;
                        target.scale = 0;
                        target.rotation = 0;
                        target.visible = true;
                        return [4 /*yield*/, getTween(target).to({
                                alpha: 1,
                                scale: 1,
                                rotation: 1080,
                            }, duration, ease)];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    }
    yyw.twirlIn = twirlIn;
    function twirlOut(target, duration, ease) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, getTween(target)
                            .to({
                            alpha: 0,
                            scale: 0,
                            rotation: -1080,
                        }, duration, ease)];
                    case 1:
                        _a.sent();
                        target.visible = false;
                        target.alpha = 1;
                        target.scale = 1;
                        target.rotation = 0;
                        return [2 /*return*/];
                }
            });
        });
    }
    yyw.twirlOut = twirlOut;
})(yyw || (yyw = {}));
